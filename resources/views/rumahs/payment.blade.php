...
<div class="container mx-auto px-4 py-12 flex justify-center items-center min-h-screen">
    <div class="bg-white shadow-md rounded-lg p-8 md:w-2/3 lg:w-1/2">
        <div class="text-center mb-6">
            ...
            {{-- <p class="text-lg text-gray-700 mb-4"><span class="font-semibold">Nama Villa:</span> {{ $rumah->nama }}</p>
            <p class="text-lg text-gray-700 mb-4"><span class="font-semibold">Lokasi:</span> {{ $rumah->lokasi }}</p>
            
            <p class="text-2xl font-bold text-green-700 mb-6">Total Harga: Rp {{ number_format($totalPrice) }}</p> --}}
...
<script type="text/javascript">
    ...
    snap.pay('{{ $snapToken }}', {
        onSuccess: function(result) {
            ...
            // Memperbarui status pembayaran ke server
            fetch('/rumah/update-status', {
                method: 'POST',
                headers: {
